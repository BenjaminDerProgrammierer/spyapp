...
import { getRandomWord, getRandomHintForWord } from './wordUtils';
import { getGameSettings } from './settingsUtils';
...
interface Game {
  ...
  word?: string;
  hintWord?: string;
  spyCount?: number; // Number of spies for this game
}
...
const createPlayerInDb = async (player: Player, gameId: string, role: 'spy' | 'regular' = 'regular'): Promise<boolean> => {
  try {
...
// Clear all games on startup
const clearAllGames = async () => {
  try {
    console.log('Clearing all games on startup...');
...
const sendRoleToPlayer = async (player: Player, gameWord: string | null, hintWord: string | null = null) => {
...
export const setupSocketHandlers = (socketIo: Server) => {
  // Store reference to io for use in helper functions
  io = socketIo;
  ...
  io.on('connection', (socket: Socket) => {
      ...
      try {
        ...
        if (playerAlreadyInGame) {
          ...
          if (game.status === 'playing') {
            ...
            if (playerInGame?.role) {        setTimeout(() => {
    ...
    const validateRoleInfoRequest = (playerId: string | null): RoleValidation => {
      if (!playerId || !players.has(playerId)) {
    ...
    // Host updates spy count for the game
    socket.on('update_spy_count', async (gameId: string, spyCount: number, callback) => {
      try {
        const playerId = socket.data.playerId;
        if (!playerId || !players.has(playerId)) {
          return callback({ success: false, error: 'Player not registered' });
        ...
        if (game.hostId !== playerId) {
          return callback({ success: false, error: 'Only the host can update spy count' });
        }
        
        if (game.status !== 'waiting') {
          return callback({ success: false, error: 'Cannot update spy count after game has started' });
        }
        
        // Validate spy count
        const validSpyCount = spyCount && spyCount >= 1 && spyCount <= 5 ? spyCount : 1;
        const maxSpies = Math.floor(game.players.length / 2);
        const actualSpyCount = Math.min(validSpyCount, maxSpies);
        
        // Update the game
        game.spyCount = actualSpyCount;
        
        // Notify all players in the game room about the spy count change
        io.to(gameId).emit('spy_count_updated', { 
          spyCount: actualSpyCount,
          maxSpies: maxSpies
        });
        
        callback({ success: true, spyCount: actualSpyCount, maxSpies: maxSpies });
      } catch (error) {
        console.error('Error updating spy count:', error);
        callback({ success: false, error: 'Failed to update spy count' });
      }
    });
  });
};
